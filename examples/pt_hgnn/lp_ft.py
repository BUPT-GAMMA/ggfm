...
'''
    parser.add_argument('--data_dir', type=str,
                        default='/home/yjy/heteroPrompt/GPT-GNN/example_OAG/data/oag_output/graph_CS.pk',
    ...
    parser.add_argument('--pretrain_model_dir', type=str,
                        default='/home/yjy/heteroPrompt/GPT-GNN/example_OAG/models/gta_all_cs3',
                        help='The address for pretrained model.')
    parser.add_argument('--model_dir', type=str, default='/home/yjy/heteroPrompt/GPT-GNN/example_OAG/models',
                        help='The address for storing the models and optimization results.')
    parser.add_argument('--task_name', type=str, default='paper_field_link_prediction',
...
def node_classification_sample(seed, pairs, time_range):
    ...
    '''
    np.random.seed(seed)
    target_ids = np.random.choice(list(pairs.keys()), args.batch_size, replace=False)  # 随机选择batch_size个target_ids
    target_info = []
...
def init():
    global target_type, types, cand_list, criterion, t, train_range, valid_range, test_range, test_pairs, sel_train_pairs, sel_valid_pairs
    target_type = 'paper'
...
'''
    ...
    for epoch in np.arange(args.n_epoch) + 1:
        ...
        model.train()
        train_losses = []
        for node_feature, node_type, edge_time, edge_index, edge_type, x_ids, ylabel in train_data:  # 对n_batch个采样子图
