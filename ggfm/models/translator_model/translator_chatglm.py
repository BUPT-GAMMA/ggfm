...
from ggfm.models.translator_model.translator import TranslatorBase
from transformers import BertTokenizer
from ggfm.models.translator_model.Qformer import BertConfig, BertLMHeadModel
from ggfm.models.chatglm2 import ChatGLMForConditionalGeneration, ChatGLMTokenizer

IMAGE_TOKEN_ID = 101


class TranslatorCHATGLMIMDB(TranslatorBase):
    ...
    def generate(
        ...
        question_prompt_pre = [prompts[1].format(t) for t in title]

        categories = [prompts[2]]*len(title)

        question_prompt = [prompts[3]]*len(title)
        qustion = [question_prompt_pre[i] + categories[i] + question_prompt[i] for i in range(len(title))]
        ...
        with self.maybe_autocast():
            ...
            from ggfm.models.chatglm2.modeling_chatglm import InvalidScoreLogitsProcessor
            ...
            for i in range(multimodal_embeds.shape[0]):
                ...
                # if not response0.strip():
                #     # 尝试缩短问题重新生成
                #     shorter_question = ' '.join(qustion[i].split()[:500])  # 截断前500词
                #     input_ids_retry, _, inputs_embeds_retry = self.prepare_lm_input(
                #         vtokens[i:i+1], qustion, answer=None
                #     )
                #     outputs_retry = self.chatglm2_model.generate(
                #         input_ids_retry,
                #         inputs_embeds=inputs_embeds_retry,
                #         **gen_kwargs
                #     )
                #     outputs_i = outputs_retry.tolist()[len(input_ids_retry):]
                #     response0 = self.chatglm2_model.process_response(
                #         self.chatglm2_tokenizer.decode(outputs_i)
                #     )
                #     try_times+=1
                if not response0.strip():
                    response0 = "action and 0"
