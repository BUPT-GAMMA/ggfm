...
def classification_func(func, output, batch):
    ...
    # print(f"func: {func(output, batch.y.view(-1).to(torch.long))}")
    # return func(output, batch.y.view(-1).to(torch.long))

    output = output.view(-1, batch.num_classes[0])
    predictions = output.argmax(dim=1)
    true_labels = batch.y.view(-1).to(torch.long)
    # Calculate macro and micro F1 scores
    macro_f1 = f1_score(true_labels.cpu(), predictions.cpu(), average='macro')
    micro_f1 = f1_score(true_labels.cpu(), predictions.cpu(), average='micro')
